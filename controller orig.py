...
class Controller:
    ...
    def update(self):
        ...
        rot_inv = np.linalg.inv(self.drone.get_transformation_matrix(
            state[3], state[4], state[5]))[:3, :3]
        #print(rot_inv)
        ...
        g = 9.81
        total_force = self.drone.m * g + self.Kp_z * \
            pos_error[2] + self.Kd_z*d_error[2]
        ...
        angle_reference = np.maximum(np.minimum(
            angle_reference, np.pi/4), -np.pi/4)
        print("Angle ref:", angle_reference)

        # Angle to forces PD

        # np.array([state[4], state[3]])
        angle_error = angle_reference - state[3:5]
